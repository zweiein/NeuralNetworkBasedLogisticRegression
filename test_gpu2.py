...
b_cpu = 2*np.identity(4).astype(np.int16)
...
# transfer host (CPU) memory to device (GPU) memory
a_gpu = gpuarray.to_gpu(a_cpu.astype(np.int16))
b_gpu = gpuarray.to_gpu(b_cpu.astype(np.int16))

# create empty gpuarry for the result (C = A * B)
#c_gpu = gpuarray.empty((MATRIX_SIZE, MATRIX_SIZE), np.complex64)

# create empty gpuarry for the result (C = A * B)
c_gpu = gpuarray.empty((2, 4), np.int16)
